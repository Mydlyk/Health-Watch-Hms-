...
export default function GuardianDetails({route,navigation}) {
    const {json,gender} = route.params;
    const {userToken, url } = useContext(AuthContext);
    const params = new URLSearchParams({
      guardianEmail: json.email.toString(),
      Authorization: "Bearer " + userToken,
    }).toString();
    console.log(route);
    const unPairGuardian = async () => {
      var polaczenie = true;
      try {
        await fetch(`http://${url}:8080/api/user/unpairGuardian?` + params, {
          method: "PUT",
          headers: {
            guardianEmail: json.email.toString(),
            Authorization: "Bearer " + userToken,
...
<Text style={styles.doctor_text}>{json.email}</Text>
      ...
      <View style={styles.container2_1}>
        <TouchableOpacity onPress={unPairGuardian} style={{ }}>
          <AntDesign
            name="close"
            size={80}
            color={"#fff"}
